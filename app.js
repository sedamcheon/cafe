...
const SAMPLE_CAFES = {
  mega_coffee: {
    ...
    menus: {
      ...
      lemon_ade: { name: "레몬에이드", category: "에이드", price: 3900, updatedAt: "2026-06-10" },
      chocolate_latte: { name: "초코라떼", category: "라떼", price: 3500, updatedAt: "2026-06-10" }
    }
  },
  compose_coffee: {
    name: "컴포즈커피",
    location: "부경대 후문 근처",
    distanceText: "부경대 후문 기준 도보 5분",
    distanceMeters: 400,
    hours: "08:00-22:30",
    tags: ["가격 저렴", "메뉴 다양"],
    updatedAt: "2026-06-10",
    menus: {
      ...
      vanilla_latte: { name: "바닐라라떼", category: "라떼", price: 3500, updatedAt: "2026-06-10" },
      ...
      strawberry_smoothie: { name: "딸기스무디", category: "스무디", price: 4300, updatedAt: "2026-06-10" }
    }
  },
  front_alley_cafe: {
    name: "학교 앞 개인카페",
    location: "부경대 정문 골목",
    distanceText: "부경대 정문 기준 도보 2분",
    distanceMeters: 160,
    hours: "09:00-21:00",
    tags: ["조용함", "공부 가능"],
    updatedAt: "2026-06-10",
    menus: {
      ...
      chocolate_latte: { name: "초코라떼", category: "라떼", price: 3300, updatedAt: "2026-06-10" },
      strawberry_smoothie: { name: "딸기스무디", category: "스무디", price: 4600, updatedAt: "2026-06-10" }
    }
  },
  station_cafe: {
    name: "역 앞 개인카페",
    location: "경성대·부경대역 3번 출구 주변",
    distanceText: "경성대·부경대역 기준 도보 3분",
    distanceMeters: 650,
    hours: "10:00-23:00",
    tags: ["좌석 많음", "늦게까지 운영"],
    updatedAt: "2026-06-10",
    menus: {
      ...
      vanilla_latte: { name: "바닐라라떼", category: "라떼", price: 3800, updatedAt: "2026-06-10" },
      grapefruit_ade: { name: "자몽에이드", category: "에이드", price: 4500, updatedAt: "2026-06-10" },
      lemon_ade: { name: "레몬에이드", category: "에이드", price: 4200, updatedAt: "2026-06-10" },
      strawberry_smoothie: { name: "딸기스무디", category: "스무디", price: 4800, updatedAt: "2026-06-10" }
    }
  },
  side_gate_takeout: {
    name: "쪽문 테이크아웃",
    location: "부경대 쪽문 버스정류장 주변",
    distanceText: "부경대 쪽문 기준 도보 1분",
    distanceMeters: 90,
    hours: "07:30-20:00",
    tags: ["빠른 주문", "테이크아웃 전문"],
    updatedAt: "2026-06-10",
    menus: {
      ...
      vanilla_latte: { name: "바닐라라떼", category: "라떼", price: 3600, updatedAt: "2026-06-10" },
      peach_iced_tea: { name: "복숭아 아이스티", category: "아이스티", price: 2700, updatedAt: "2026-06-10" },
      chocolate_latte: { name: "초코라떼", category: "라떼", price: 3200, updatedAt: "2026-06-10" },
      lemon_ade: { name: "레몬에이드", category: "에이드", price: 3500, updatedAt: "2026-06-10" }
    }
  },
  library_lounge: {
    name: "도서관 라운지카페",
    location: "부경대 대연캠퍼스 도서관 방향",
    distanceText: "부경대 정문 기준 도보 7분",
    distanceMeters: 520,
    hours: "09:00-19:00",
    tags: ["학내 이동 편함", "간단한 디저트"],
    updatedAt: "2026-06-10",
    menus: {
      ice_americano: { name: "아이스 아메리카노", category: "커피", price: 2200, updatedAt: "2026-06-10" },
      cafe_latte: { name: "카페라떼", category: "라떼", price: 3100, updatedAt: "2026-06-10" },
      peach_iced_tea: { name: "복숭아 아이스티", category: "아이스티", price: 3000, updatedAt: "2026-06-10" },
      grapefruit_ade: { name: "자몽에이드", category: "에이드", price: 4100, updatedAt: "2026-06-10" },
      strawberry_smoothie: { name: "딸기스무디", category: "스무디", price: 4500, updatedAt: "2026-06-10" },
      chocolate_latte: { name: "초코라떼", category: "라떼", price: 3400, updatedAt: "2026-06-10" }
    }
  }
};
...
function handleDocumentClick(event) {
  ...
  if (categoryButton) {
    ...
    const firstDrink = getVisibleDrinks()[0] || "";
    selectDrink(firstDrink, { shouldRenderDrinks: false, shouldCloseDetail: true });
    renderCategories();
    renderDrinks();
    renderRanking();
    return;
  ...
  if (drinkButton) {
    selectDrink(drinkButton.dataset.drink, { shouldCloseDetail: true });
    return;
  }
...
async function loadCafeData() {
  ...
  try {
    ...
    state.rawCafeData = data || null;
    state.needsDevelopmentSeed = needsDevelopmentSeed(data);
...
function needsDevelopmentSeed(data) {
  const cafes = normalizeCafes(data || {});
  const pricedMenus = cafes.flatMap((cafe) => Object.values(cafe.menus).filter((menu) => isKnownPrice(menu.price)));

  if (cafes.length < 6 || pricedMenus.length < 24) {
    return true;
  }

  return REQUIRED_SAMPLE_DRINKS.some((drink) => {
    const count = cafes.reduce((total, cafe) => {
      const hasDrink = Object.values(cafe.menus).some((menu) => normalizeText(menu.name) === normalizeText(drink) && isKnownPrice(menu.price));
      return total + (hasDrink ? 1 : 0);
    }, 0);
    return count < 3;
  });
}

async function seedDevelopmentDataIfNeeded() {
  if (!hasFirebaseConfig() || !state.isAdmin || !state.needsDevelopmentSeed) return false;

  const seedPatch = buildDevelopmentSeedPatch(state.rawCafeData);
  if (!Object.keys(seedPatch).length) return false;

  try {
    const token = await getFreshIdToken();
    await dbPatch("cafes", seedPatch, token);
    await reloadCafesOnly();
    showToast("개발용 예시 카페 데이터를 Firebase에 보강했습니다.");
    return true;
  } catch (error) {
    console.warn("Development seed skipped", error);
    return false;
  }
}

function buildDevelopmentSeedPatch(existingData) {
  const existing = existingData || {};
  const existingNames = new Set(
    Object.values(existing)
      .map((cafe) => normalizeText(cafe && cafe.name))
      .filter(Boolean)
  );

  return Object.entries(SAMPLE_CAFES).reduce((patch, [cafeId, cafe]) => {
    if (!existing[cafeId] && !existingNames.has(normalizeText(cafe.name))) {
      patch[cafeId] = cafe;
    }
    return patch;
  }, {});
...
function handleStreamEvent(event) {
  try {
    ...
    if (payload.path === "/" && payload.data) {
      state.rawCafeData = payload.data;
      state.needsDevelopmentSeed = needsDevelopmentSeed(payload.data);
...
function selectDrink(drinkName, options = {}) {
  const shouldRenderDrinks = options.shouldRenderDrinks !== false;
  const shouldCloseDetail = options.shouldCloseDetail !== false;
  state.selectedDrink = String(drinkName || "").trim();
  state.selectedCafeId = "";

  if (shouldRenderDrinks) {
    renderDrinks();
  }
  renderRanking();
...
function renderDrinks() {
  ...
  el.drinkGrid.innerHTML = drinks
    .map((drink) => {
      ...
      const minPrice = rows.map((row) => row.price).sort((a, b) => a - b)[0];
...
function renderRanking() {
  ...
  if (!drink) {
    el.rankingHeader.innerHTML = `
      <div>
        ...
        <h2>먹고 싶은 음료를 선택해주세요</h2>
        <p>왼쪽 음료 카드나 검색 결과를 누르면 최저가 순위가 표시됩니다.</p>
      </div>
    `;
    el.rankingList.innerHTML = `<div class="empty-state">먹고 싶은 음료를 선택해주세요.</div>`;
    return;
  }
  ...
  if (!rows.length) {
    el.rankingList.innerHTML = `<div class="empty-state">해당 음료 가격 정보가 없습니다.</div>`;
    return;
  }
...
function renderRankingCard(row, index) {
  ...
  return `
    <button class="ranking-card" type="button" data-ranking-cafe-id="${escapeAttr(row.cafe.id)}">
      <span class="rank-badge">${escapeHtml(`${index + 1}위`)}</span>
      <span class="ranking-main">
        <strong>${escapeHtml(row.cafe.name)}</strong>
        <span>${escapeHtml(row.name)} ${escapeHtml(formatPrice(row.price))}</span>
        <span>${escapeHtml(row.cafe.distanceText || "거리 확인 필요")}</span>
        <span class="tag-row">${features}</span>
      </span>
      <span class="price-block">
        <strong>${escapeHtml(formatPrice(row.price))}</strong>
        <span>최근 수정: ${escapeHtml(row.updatedAt || row.cafe.updatedAt || "확인 필요")}</span>
      </span>
    </button>
  `;
}
...
async function handleCafeSave(event) {
  ...
  const cafe = {
    ...
    location: el.cafeLocation.value.trim() || el.cafeArea.value.trim(),
    locationDetail: el.cafeLocation.value.trim() || el.cafeArea.value.trim(),
    distanceText: el.cafeDistanceText.value.trim(),
    distanceMeters: Number(el.cafeDistanceMinutes.value) * 80,
    distanceMinutes: Number(el.cafeDistanceMinutes.value),
    hours: el.cafeHours.value.trim() || "확인 필요",
    tags: parseFeatures(el.cafeFeatures.value),
...
function getRankingsForDrink(drink) {
  ...
  const rows = flattenMenus().filter((item) => normalizeText(item.name) === wanted && isKnownPrice(item.price));

  return rows.sort((a, b) => {
    if (a.price !== b.price) return a.price - b.price;

    const aDistance = getComparableDistance(a.cafe);
    const bDistance = getComparableDistance(b.cafe);
    if (aDistance !== bDistance) return aDistance - bDistance;

    return a.cafe.name.localeCompare(b.cafe.name, "ko");
  });
}
...
function normalizeCafes(data) {
  return Object.entries(data || {})
    .map(([id, raw]) => {
      ...
      const distanceMeters = normalizeDistanceMeters(raw);
      return {
        id,
        name: String(raw.name || "이름 확인 필요"),
        area: String(raw.area || raw.location || ""),
        location: String(raw.location || raw.locationDetail || raw.area || ""),
        locationDetail: String(raw.location || raw.locationDetail || raw.area || ""),
        distanceText: String(raw.distanceText || raw.distance || "거리 확인 필요"),
        distanceMeters,
        distance: normalizeDistanceValue(raw.distance),
        ...
        features: normalizeFeatures(raw.tags || raw.features),
        updatedAt,
        menus
      };
    })
    .sort((a, b) => getComparableDistance(a) - getComparableDistance(b) || a.name.localeCompare(b.name, "ko"));
}
...
function normalizeFeatures(features) {
  ...
  if (features && typeof features === "object") {
    const cleaned = Object.values(features).map((feature) => String(feature).trim()).filter(Boolean);
    return cleaned.length ? cleaned : ["정보 확인 필요"];
  }
  return ["정보 확인 필요"];
}
...
function normalizeDistanceMeters(raw) {
  const directMeters = Number(raw.distanceMeters);
  if (Number.isFinite(directMeters)) return directMeters;

  const distance = normalizeDistanceValue(raw.distance);
  if (Number.isFinite(distance)) return distance;

  const minutes = Number(raw.distanceMinutes ?? parseDistanceMinutes(raw.distanceText));
  return Number.isFinite(minutes) ? minutes * 80 : Number.POSITIVE_INFINITY;
}
...
function getComparableDistance(cafe) {
  const meters = Number(cafe.distanceMeters);
  if (Number.isFinite(meters)) return meters;

  const distance = Number(cafe.distance);
  if (Number.isFinite(distance)) return distance;

  const minutes = Number(cafe.distanceMinutes);
  if (Number.isFinite(minutes)) return minutes * 80;
